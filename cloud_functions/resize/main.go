...
func init() {
	imagick.Initialize()
	functions.HTTP("ResizeImage", ResizeImageHandler)
}
...
func TestMain(t *testing.T) {
	...
	// テスト用のリクエストを作成
	req, err := http.NewRequest("GET", "https://localhost:8080/resize?image_name=./images/image.jpg&w=100", nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	// 画像の保存
	if err := os.WriteFile("./images/image-resized.jpg", b, 0666); err != nil {
		t.Fatal(err)
	}
